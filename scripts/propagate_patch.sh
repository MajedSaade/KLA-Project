...
#
# Finds the definitive fix commit (not every WI-tagged commit) and cherry-picks
# it onto branches that contain the affected file but lack the fix.
#
# Usage:
...
#   AFFECTED_FILE          File the patch modifies (default: src/payment/transaction_queue.py)
